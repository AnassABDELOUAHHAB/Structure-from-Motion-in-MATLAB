function [Rwi_c, Twi_c, Uw] = bundleAdjustment_LM_n_views_full(Rwi_c, Twi_c, Uw, tracks_cur, K, p, nTracks, maxIt)
    nCam = size(Rwi_c,2);
    lambda = 1e-3;
    lambdaMin = 1e-5;
    lambdaMax = 1e5;
    %compute cost
    c = compute_cost(Rwi_c, Twi_c, Uw, tracks_cur, K, p,nCam);
    fprintf('Iter:\t Error:\t \t lambda:\n');
    fprintf('%d\t%f pix \t%f\n',0,c,lambda); 
    for i = 1:maxIt     
        %compute Jacobian and residuals
        r = compute_residuals(Rwi_c, Twi_c, Uw, tracks_cur, K, p,nCam);
        J = compute_Jacobian(Rwi_c, Twi_c, Uw, tracks_cur, K, nTracks,nCam);
        %update parameters
...
function c = compute_cost(Rwi_c, Twi_c, Uw, tracks_cur, K, p,nCam)
    r = compute_residuals(Rwi_c, Twi_c, Uw, tracks_cur, K, p,nCam);
    c = r'*r;
    nb_Pt_2D = 0;
...
function r = compute_residuals(Rwi_c, Twi_c, Uw, tracks_cur, K, p,nCam)
    r = [];
    for i = 1:nCam
...
function J = compute_Jacobian(Rwi_c, Twi_c, Uw, tracks_cur, K, nTracks,nCam)
    nb_Pt_2D = 0;
    for i = 1:nCam %for each camera    
        nb_Pt_in_current_cam = length(tracks_cur{i}.point3D_ids);
        nb_Pt_2D = nb_Pt_2D + nb_Pt_in_current_cam;
    ...
    for i = 1:nCam %for each camera
        ...
        for t = 1 :nb_Pt_in_current_cam %for each 2D point
            ...
            u = [u, (l+x1)', (l+x2)', (l+x3)'];
            v = [v, (y1+3*(tracks_cur{i}.point3D_ids(t) - 1))', (y2-1+3*nTracks+1+(i-1)*6)', (y3-1+3*nTracks+4+(i-1)*6)'];
...
function [Rwi_new_c, Twi_new_c, Uw_new, c_new, lambda, success] = updateParameters(Rwi_c, Twi_c, Uw, tracks_cur, K, p, nTracks, r, J, lambda, lambdaMax, lambdaMin, c_prev, iter,nCam)
    ...
    while(lambda<lambdaMax)
        ...
        Rwi_new_c = cell(1, nCam);
        Twi_new_c = cell(1, nCam);
        for i=1:nCam
            idx_rot = 3*nTracks+1+(i-1)*6;
            idx_trans = idx_rot+3; 

            Rwi_new_c{i} = Rwi_c{i} * expm(HatSO3(delta(idx_rot:idx_rot + 2)));
            Twi_new_c{i} = Twi_c{i} + delta(idx_trans:idx_trans + 2);
        end
        %compute cost
        c_new = compute_cost(Rwi_new_c, Twi_new_c, Uw_new, tracks_cur, K, p,nCam);
        fprintf('%d\t%f pix\t%f\n',iter,c_new,lambda);
